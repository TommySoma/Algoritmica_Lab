...
int ConnessoEAciclico(grafo *a, int n);
...
int ConnessoEAciclico(grafo *a, int n){
	...
	for(i=0;i<n;i++)			//lo inizializzo tutto a 0
		c[i]=0;
	DFS(a,0,&c);				//richiamo la dfs partendo dal primo nodo per vedere se è connesso
	i=0;
	while(i<n){
...
void DFS(grafo *a, int cerc, int **c){
	...
	for(i=0;i<a[cerc].info;i++){		//cerco nei nodi adiacenti
		...
		else return;			//altrimenti l'ho già visitato e presenta un ciclo
	}
